...
namespace ProjectGotham
{
    ...
    public class StartUp : AsyncResource
    {
        ...
        private void ConfigureLogging()
        {
            ...
            .ReadFrom.Configuration(Configuration, "Serilog", ConfigurationAssemblySource.AlwaysScanDllFiles)
            .CreateLogger();
        }
        ...
        private async Task InvokeServiceAsync<T>(Func<T, Task> action) where T : class
        {
            ...
            if (service != null)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    logger?.Error(ex, "Error while invoking service");
                }
            }
            else
            {
                ILogger? logger = ServiceProvider.GetService<ILogger>();
                logger?.Error("Service not found while invoking");
            }
        }
